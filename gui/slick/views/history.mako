                                    ...
                                    <td align="center" provider="${str(sorted(hItem["actions"])[0]["provider"])}">
                                        % for action in sorted(hItem["actions"]):
                                            ...
                                            % if curStatus in [SNATCHED, FAILED]:
                                                ...
                                                % if provider is not None:
                                                    <img src="${static_url('images/providers/' + provider.image_name())}" width="16" height="16" style="vertical-align:middle;" alt="${provider.name}" style="cursor: help;" title="${provider.name}: ${ek(os.path.basename, action["resource"])}"/>
                                                % else:
                                    ...
                                    % if sickbeard.USE_SUBTITLES:
                                        <td align="center">
                                            % for action in sorted(hItem["actions"]):
                                                <% curStatus, curQuality = Quality.splitCompositeStatus(int(action["action"])) %>
                                                % if curStatus == SUBTITLED:
                                                    <img src="${static_url('images/subtitles/' + action['provider'] + '.png')}" width="16" height="16" style="vertical-align:middle;" alt="${action["provider"]}" title="${action["provider"].capitalize()}: ${ek(os.path.basename, action["resource"])}"/>
